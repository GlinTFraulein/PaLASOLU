...
namespace PaLASOLU
{
	...
	public partial class LoweffortUploaderCore : Plugin<LoweffortUploaderCore>
	{
		...
		/*
		public AudioClip CutClipbyTime(AudioClip audio, TimelineClip nowClip, TimelineAsset timeline)
		{
			double startTime = nowClip.clipIn;
			double duration = nowClip.duration;
			double timeScale = nowClip.timeScale;

			if (startTime == 0 && System.Math.Abs((duration * timeScale - audio.length)) < 0.01) return audio;
